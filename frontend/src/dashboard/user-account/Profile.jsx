...
const Profile = ({user, token}) => {
  ...
  useEffect(()=>{
    setFormData({name: user.name, email:user.email, gender: user.gender, photo: user.photo, bloodType: user.bloodType})
  }, [user])
  ...
  return (
    <div className='mt-10'>
        <form className='py-4 md:py-0' onSubmit={handleSubmit}>
                ...
                <div>
                  <button disabled={loading && true} type="submit" className='w-full bg-primaryColor text-white text-[18px] rounded-lg p-2'>{loading ? <HashLoader size={35} color='white' /> : 'Update'}</button>
                </div>
            </form>
    </div>
  )
